...
export async function getListTanyaJawab(payload) {
  ...
  const res = await fetch(`${process.env.BACKEND_URL}/tanya_jawab/list`, {
    method: "POST",
    headers: headers,
...
export async function saveTanyaJawab(payload) {
  const cookie = (await cookies()).get("session")?.value;
  const session = await decrypt(cookie);
  const headers = new Headers();
  headers.append("Authorization", session?.userId);
  ...
  const res = await fetch(`${process.env.BACKEND_URL}/tanya_jawab/add`, {
    method: "POST",
    headers: headers,
    body: JSON.stringify(payload),
  });
