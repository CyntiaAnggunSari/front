...
import { getListGuru, deleteGuru } from "@/app/actions/guru";
...
export default function GuruPage() {
  ...
  const editHandler = (data) => {
    return redirect(`/dashboard/guru/edit?search=${data.nip}`);
  };
  ...
  const deleteDataGuru = async () => {
    setIsLoading(true);
    try {
      const data = { nip: nip };
      const result = await deleteGuru(data);
  ...
  const sortedDataGuru = [...dataGuru].sort((a, b) =>
            ...
            <Paper sx={{ width: "100%", overflow: "hidden" }}>
              ...
              <TablePagination
                ...
                slotProps={{
                  actions: {
                    nextButton: {
                      disabled: dataGuru.length < payload.limit,
                    },
                    previousButton: {
