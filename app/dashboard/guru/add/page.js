...
import { saveGuru } from "@/app/actions/guru";
...
export default function AddGuruPage() {
  ...
  const [formState, formAction] = useActionState(saveGuru, {
    status: 0,
    message: "",
  });
  ...
  const nip = nipRef.current?.value.trim();
  ...
  if (!nip || !nama || !jenisKelamin || !alamat || !agama || !noHp || !password) {
