...
import {
  getListTugasMatapelajaranGuru,
  getListMatapelajaranSiswa,
} from "@/app/actions/matapelajaran";

export default function TugasKelasPage() {
  ...
  useEffect(() => {
    ...
    } else if (roleId == 3 && payload.tahun !== "") {
      getDataMatapelajaran();
    }
  }, [payload]);
  ...
  const getDataMatapelajaran = async () => {
    ...
    try {
      if (roleId == 2) {
        const listMatapelajaran = await getListTugasMatapelajaranGuru(payload);
        ...
        if (!payload.tahun) return;
        const listMatapelajaran = await getListMatapelajaranSiswa(payload);
      ...
      <div className="mt-2 w-full">
        ...
        <div className="grid grid-cols-4 justify-start gap-4 mt-16">
          {dataMatapelajaran.map((mp) => (
            <Link
              key={mp.id_mp}
              href={`/dashboard/tugas/${kelas}/${mp.id_mp}`}
